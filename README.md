...
1. Choose ubuntu machine.
2. t2.small is an instance type.
3. If you want to connect with putty, create a .pem file.
4. For network select http, https and allow from anywhere.
5. Now review the settings and launch the machine.
...
5. To activate runner, execute below command
   ...
   ./run.sh
    ```
6. Now go to do any changes in the repository code and push it. It will start the workflow.
7. Now go to EC2 dashboard and open public DNS url and then remove **_`s`_** from **`https`** and add port number at the end of the url i.e. :8080
8. Add /train in the url, if you to train the model or add /predict if you want to predict.
