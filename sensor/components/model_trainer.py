    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        try:
            ...
            if classification_train_metric.f1_score <= self.model_trainer_config.expected_accuracy:
                raise Exception("Trained model is not good to provide excepted accuracy")
            ...
            model_dir_path = os.path.dirname(self.model_trainer_config.trained_model_fie_path)
            os.makedirs(model_dir_path, exist_ok=True)
            sensor_model = SensorModel(preprocessor=preprocessor, model= model)
            save_object(self.model_trainer_config.trained_model_fie_path, obj= sensor_model)
